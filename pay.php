...
if(isset($_REQUEST['orderid'])){
 $res= getOrderById($conn, $_REQUEST['orderid']);
 $row= mysqli_fetch_assoc($res);
 $price= $row['order_price'];
$cid= $row['user_id'];

$customerdata= getCustomerById($conn, $cid);
$crow= mysqli_fetch_assoc($customerdata);
$name= $crow['Name'];
$email= $crow['Email'];
$contact= $crow['Phone'];
...
function getOrderById($conn, $oid){
    $sql= "SELECT * FROM delivery_address_detail WHERE order_id= '$oid'";
    $res= mysqli_query($conn, $sql);
    return $res;

}

function getCustomerById($conn, $cid){
    $sql= "SELECT * FROM users_data where user_id= '$cid'";
    $res= mysqli_query($conn, $sql);
    return $res;
}
